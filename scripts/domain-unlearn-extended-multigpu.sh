...
##############################################################################
# Domain Unlearning Pipeline - MULTI-GPU VERSION
#
# This script performs end-to-end domain unlearning using MULTIPLE GPUs:
...
#
# Usage:
#   bash scripts/domain-unlearn-extended-multigpu.sh <TOPIC> [MODEL] [TRAINER] [FLAGS]
#
# Flags:
...
#
# Example:
#   bash scripts/domain-unlearn-extended-multigpu.sh "Brazil"
#   bash scripts/domain-unlearn-extended-multigpu.sh "Brazil" Llama-3.2-3B-Instruct NPO --skip-generation
#
##############################################################################
...
# Fine-tuning hyperparameters (smaller batch for multi-GPU)
FINETUNE_EPOCHS=3
FINETUNE_LEARNING_RATE=1e-5
FINETUNE_BATCH_SIZE=2                    # Smaller per-GPU batch
FINETUNE_GRADIENT_ACCUMULATION=4         # Effective: 2 * 4 * num_gpus

# Unlearning hyperparameters
UNLEARN_EPOCHS=3
UNLEARN_LEARNING_RATE=1e-5
UNLEARN_BATCH_SIZE=2                     # Smaller per-GPU batch
UNLEARN_GRADIENT_ACCUMULATION=8          # Effective: 2 * 8 * num_gpus
...
echo "================================================================================================"
echo "Domain Unlearning Pipeline - MULTI-GPU VERSION (${NUM_GPUS} GPUs)"
echo "================================================================================================"
...
echo ""
echo "Fine-tuning Configuration (MULTI-GPU):"
echo "  Epochs:             ${FINETUNE_EPOCHS}"
echo "  Batch Size/GPU:     ${FINETUNE_BATCH_SIZE}"
echo "  Gradient Accum:     ${FINETUNE_GRADIENT_ACCUMULATION}"
echo "  Effective Batch:    $((FINETUNE_BATCH_SIZE * FINETUNE_GRADIENT_ACCUMULATION * NUM_GPUS))"
echo "  Learning Rate:      ${FINETUNE_LEARNING_RATE}"
echo "  GPUs:               ${NUM_GPUS}"
echo ""
echo "Unlearning Configuration (MULTI-GPU):"
echo "  Epochs:             ${UNLEARN_EPOCHS}"
echo "  Batch Size/GPU:     ${UNLEARN_BATCH_SIZE}"
echo "  Gradient Accum:     ${UNLEARN_GRADIENT_ACCUMULATION}"
echo "  Effective Batch:    $((UNLEARN_BATCH_SIZE * UNLEARN_GRADIENT_ACCUMULATION * NUM_GPUS))"
...
##############################################################################
# Step 6: Fine-tune Model on Domain Data (MULTI-GPU)
##############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Step 6: Fine-tuning ${MODEL} on Domain Data (MULTI-GPU: ${NUM_GPUS} GPUs)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
##############################################################################
# Step 7: Run Unlearning on Finetuned Model (MULTI-GPU)
##############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Step 7: Running Unlearning with ${TRAINER} on Finetuned Model (MULTI-GPU: ${NUM_GPUS} GPUs)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
##############################################################################
# Step 8: Run Evaluation (Compare all 3 models) - Single GPU for eval
##############################################################################
...
echo "================================================================================================"
echo "Domain Unlearning Pipeline Complete! 🎉 (MULTI-GPU: ${NUM_GPUS} GPUs)"
echo "================================================================================================"
echo ""
echo "Summary:"
echo "  Topic:                ${TOPIC}"
